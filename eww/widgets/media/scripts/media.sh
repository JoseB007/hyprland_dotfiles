...
playerctl --all-players metadata \
    --format $'{{playerName}}\x1F{{title}}\x1F{{artist}}\x1F{{album}}\x1F{{mpris:artUrl}}\x1F{{position}}\x1F{{mpris:length}}\x1F{{status}}' \
    --follow 2>/dev/null | while IFS=$'\x1F' read -r player title artist album artUrl position length status; do
